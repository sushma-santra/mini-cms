...
export default function PostEditor({ initialData, onSave, onCancel, isLoading }: PostEditorProps) {
  ...
  const [fullText, setFullText] = useState(initialData?.fullText || initialData?.content || '')
  ...
  // ReactQuill modules configuration for description (simpler toolbar)
  const descriptionQuillModules = {
    toolbar: [
  ...
  // Simpler formats for description
  const descriptionQuillFormats = [
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!fullText.trim()) {
      alert('Please add some content to your post. The content field is required.')
      return
    }
    ...
    const data = {
      ...
      featuredImage: validImages.length > 0 ? validImages[0].url : null,  // Set to null instead of undefined when no images
      ...
      })),  // Always send array, empty if no images
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
            ...
            {descriptionEditorMode === 'visual' ? (
              <div className="border border-gray-300 rounded-lg overflow-hidden bg-white description-editor">
                <ReactQuill
                  ...
                  formats={descriptionQuillFormats}
                  className="bg-white"
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="block w-full px-4 py-3 text-gray-900 placeholder-gray-500 border border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 focus:ring-1 transition-colors"
              placeholder="External links (optional)"
            />
          </div>
