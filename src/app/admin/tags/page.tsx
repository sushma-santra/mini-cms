...
export default function TagsPage() {
  ...
  const handleCreateTag = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to ${isEditing ? 'update' : 'create'} tag`)
      }

      resetForm()
      fetchTags()
    } catch (error) {
      console.error(`Error ${editingTagId ? 'updating' : 'creating'} tag:`, error)
      alert(`Failed to ${editingTagId ? 'update' : 'create'} tag`)
    }
  }
      ...
      <div className="flex justify-between items-center border-b border-gray-200 pb-4 mb-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Create and manage tags to organize and categorize your blog posts.
          </p>
        </div>
        <button
          onClick={() => {
      ...
      {showCreateForm && (
        <div className="bg-white shadow-sm border border-gray-200 rounded-lg overflow-hidden mb-4">
          <div className="px-4 py-3 border-b border-gray-200 bg-gray-50">
            <h2 className="text-sm font-medium text-gray-900">{editingTagId ? 'Edit Tag' : 'Create New Tag'}</h2>
          </div>
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    placeholder="e.g., viratkohli, WorldCup2024, InjuryUpdate"
                    required
                  />
                </div>
      ...
      {tags.length === 0 ? (
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-md">
          <ul className="divide-y divide-gray-200">
            {tags.map((tag: { id: string; name: string; description?: string; posts?: any[] }) => (
              <li key={tag.id} className="hover:bg-gray-50 transition-colors">
